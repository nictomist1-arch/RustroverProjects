...
fn save_attachment(app: tauri::AppHandle, source: String) -> Result<String, String> {
    ...
    if !source_path.is_file() {
        return Err("Выбранный файл не существует".to_string());
    }
    ...
    let extension = source_path
        ...
        .ok_or_else(|| "У файла нет расширения".to_string())?;

    let allowed_extensions = ["png", "jpg", "jpeg", "webp", "gif"];

    if !allowed_extensions.contains(&extension.as_str()) {
        return Err("Этот формат изображения не поддерживается".to_string());
    }
    ...
    let saved_path = destination
        .to_str()
        .ok_or_else(|| "Не удалось преобразовать путь файла".to_string())?
        .to_string();
...
pub fn run() {
    let migrations = vec![
        ...
        Migration {
            ...
            sql: include_str!("../migrations/0004_message_attachments.sql"),
            kind: MigrationKind::Up,
        },
    ];
